...
void scanWiFiNetworks() {
  ...
  // Окно сканирования
  tft.fillRect(TFT_WIDTH_SCREEN / 2 - 120, 80, 240, 80, TFT_BLACK);
  tft.drawRect(TFT_WIDTH_SCREEN / 2 - 120, 80, 240, 80, TFT_GREEN);
  tft.setTextColor(TFT_GREEN);
  tft.setTextDatum(MC_DATUM);
  tft.setTextSize(1);
  tft.drawString("SCANNING...", TFT_WIDTH_SCREEN / 2, 120);
...
void connectToWiFi(const char* ssid, const char* password) {
  ...
  tft.fillRect(TFT_WIDTH_SCREEN / 2 - 120, 80, 240, 80, TFT_BLACK);
  tft.drawRect(TFT_WIDTH_SCREEN / 2 - 120, 80, 240, 80, TFT_GREEN);
  tft.drawString("CONNECTING TO:", TFT_WIDTH_SCREEN / 2, 90);
  tft.drawString(ssid, TFT_WIDTH_SCREEN / 2, 110);
  tft.setTextColor(tft.color565(0, 35, 0));
  tft.drawString(password, TFT_WIDTH_SCREEN / 2, 120);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.drawRect(TFT_WIDTH_SCREEN / 2 - 120, 80, 240, 80, TFT_GREEN);
    tft.setTextColor(TFT_GREEN);
    tft.drawString("CONNECTED!", TFT_WIDTH_SCREEN / 2, 100);
    if (DEBUGFLAG) Serial.printf("[WiFi] Connected to %s.\n", connectedSSID);
    tft.drawString(WiFi.localIP().toString(), TFT_WIDTH_SCREEN / 2, 130);
    delay(2000);
  } else {
    tft.fillRect(TFT_WIDTH_SCREEN / 2 -120, 80, 240, 80, TFT_BLACK);
    tft.drawRect(TFT_WIDTH_SCREEN / 2 -120, 80, 240, 80, TFT_GREEN);
    ...
    tft.drawString("FAILED!", TFT_WIDTH_SCREEN / 2, 110);
    tft.setTextSize(1);
    delay(2000);
  }
